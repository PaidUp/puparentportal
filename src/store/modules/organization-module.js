  ...
  return invoices.reduce((val, current) => {
    ...
    if (prd) {
      ...
      prd = {
        ...
        firstName: current.beneficiaryFirstName,
        lastName: current.beneficiaryLastName,
        total: current.price,
        paid: 0,
        unpaid: 0,
        overdue: 0,
...
const module = {
  ...
  actions: {
    ...
    getReducePrograms (context, { organizationId, seasonId }) {
      return Promise.all([
    ...
    getReducePlayers (context, { organizationId, seasonId, productId }) {
      return Promise.all([
        ...
        let items = reduceInvoicePlayers(values[0])
        reduceCreditPlayers(values[1], items)
        reducePreorderPlayers(values[2], items)
        return items
      })
    }
  }
